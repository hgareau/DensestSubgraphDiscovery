...
public:
    ...
    MDS DynamicExact() {
        Component* C;
        Component* index = queue.front();
        double low_bound = 0;
        std::queue<Component*> temp_queue = queue;
        ...
        MDS mds(std::vector<int>(), index->motif_num, index->graph_size, low_bound);

        while (!queue.empty()) {
            C = queue.front();
            queue.pop();

            ExactAlgo exact = Exactalgo(C->motif_list, motif_size, C->graph_size, C->motif_degree);
            ...
            low_bound = static_cast<double>(motif_num) / (vertex_num * 1.0);
