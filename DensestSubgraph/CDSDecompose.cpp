...
CDSDecompose::CDSDecompose(vector<vector<int>>& Graph, vector<vector<int>>& Motif, int graph_size, int motif_size, int motif_type,
    ...
    // initialize the size of degree for every vertex of the motif
    degree.resize(motif_size);
    ...
    Share.resize(graph_size);
    for (int i = 0; i < graph_size; ++i) {
        Share[i] = unordered_map<int, int>();
    }
}

vector<vector<double>> CDSDecompose::Decompose()
{
    ...
    vector<long> array_degree(graph_size);

    //empty array used to create pruned maps
    vector<int> new_array(graph_size, 0);
    //empty map used to create pruned maps
    vector<int> new_map(graph_size, 0);
    ...
    vector<long> temp_degree(graph_size);
    for (int i = 0; i < graph_size; ++i) {
    ...
    //4 = unused
    vector<vector<double>> result(graph_size + 1, vector<double>(5));
    ...
    vector<int> pos(graph_size + 1, 0); //holds vertex position in bin
    vector<int> vert(graph_size + 1, 0); //holds the vertex at that position
    ...
    for (int i = 0; i < graph_size; ++i) {
        ...
        if (array_degree[index] > 0) {
            ...
            result[count][3] = motif_num;
            //if the number of vertices in the graph - the number of vertices checked
            //is > 0, set the 3rd value in count's result array as motif num divided by that
            //Otherwise, set that value to 0
            if (graph_size - count > 0) {
                result[count][2] = motif_num / (double)(graph_size - count);
            }
...
unordered_map<int, long> CDSDecompose::Generate(int index, vector<int> &mark, vector<int> &array, vector<int> &map_s)
{
    vector<int> temp_list; // create temporary vector
    ...
    vector<int> map_array(count); // map_array has size of same number as above
    int num = 0;
    vector<vector<int>> new_graph(count); // the new subgraph with the non-pruned vertices
    ...
    vector<long> t_a = f.getMotifDegree();
    // create new map result which will be returned as final product
    unordered_map<int, long> result;
